...
describe("buildGroundingCard", () => {
  ...
  it("shows taken medication with check mark", () => {
    const card = buildGroundingCard({
      ...base,
      todayMedications: [{ name: "Aspirin", dosage: "5mg", status: "taken" }]
    });
    expect(card).toContain("Aspirin");
  ...
  it("shows pending medication with pill emoji", () => {
    const card = buildGroundingCard({
      ...base,
      todayMedications: [{ name: "Aspirin", dosage: "5mg", status: "pending" }]
    });
    expect(card).toContain("💊");
  });
